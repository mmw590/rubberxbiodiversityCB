...
# Comparing the average rubber bioclimatic suitability of converted land area across the different scenarios, Scenario 2 (Conservation) only converted areas with suitability values close to zero (mean ± 1SE = 0.0001 ± 0.000004; Figure 3A and 3C). The conservation-driven scenario is thus impractical for rubber expansion, with poor suitability leading to lower yields and higher land area demands to meet production targets. While not as high as the production-driven scenario (0.667 ± 0.011 / 0.629 ± 0.012), the average suitability of areas converted in all three compromise scenarios remained high (3a = 0.635 ± 0.011; 3b = 0.573 ± 0.010; 3c = 0.629 ± 0.011; Figure 3A / (3a = 0.595 ± 0.0123; 3b = 0.529 ± 0.008; 3c = 0.591 ± 0.012; Figure 3D), suggesting higher acceptability to rubber producers. 
...
# + Numbers for Results (unrestricted) ####
...
# Average suitability of converted land in Scenarios 1 and 3a-c was 6-8% higher in unrestricted simulations (Figure 3D). 
...
cowplot::save_plot("output/results/Fig3_A-F.png", scenfig_comb, base_width=6.85, base_height = 6.85/3*2, dpi=300)
...
# NUMBERS FOR METHODS ####
# We ran the conversion simulations on 1155 forest dependent amphibian species, 2376 bird species, and 733 mammal species occurring in our study region. 
...
dflist <- lapply(dflist[c(1:5)], fread)

spprangeloss_df <- rbindlist(dflist)
str(spprangeloss_df)

spprangeloss_df <- spprangeloss_df %>% mutate(scenario = rep(c('1-Production', '2-Conservation','3a-Compromise Biodiversity', '3b-Compromise Suitability', '3c-Compromise Both'), each=nrow(dflist[[1]]) )) 
...
spprangeloss_sum <- spprangeloss_df %>% group_by(scenario, iucn_category) %>% 
...
#Under Scenario 1 (Production), 21-43 species are predicted to lose ≥10% of their forested range under 2.45-3.90 Mha of expansion, of which 1-3 species would lose ≥50% of their forested range (Figure 4), 
#whereas no species would be affected in Scenario 2 (Conservation). 
#Scenario 3a (Compromise biodiversity) would reduce the number of affected species to 8-32,
#whilst Scenarios 3b (Compromise suitability) and 3c (Compromise both simultaneously) would further reduce this number to 6-15 and 8-16 respectively
#Scenario 3a also caused 1-2 species to lose ≥50% of their forested range, but the other compromise scenarios (3b & 3c) would drive no such declines (Figure 4). 
...
##### **** COUNTRY SCENARIOS **** ####
# Load spprangeloss tbls from simulations & merge with sppnames/iucn categories ####
#dflist <- list.files('output/results/checking/', full.names=TRUE)
...
#Under Scenario 1 (Production), 21-43 (46-74) species are predicted to lose ≥10% of their forested range under 2.45-3.90 Mha of expansion, of which 1-3 (4-6) species would lose ≥50% of their forested range (Figure 4), 
#whereas no species would be affected in Scenario 2 (Conservation). 
#Scenario 3a (Compromise biodiversity) would reduce the number of affected species to 8-32 (18-25),
#whilst Scenarios 3b (Compromise suitability) and 3c (Compromise both simultaneously) would further reduce this number to 6-15 (8-25) and 8-16 (14-25) respectively
#Scenario 3a also caused 1-2 species to lose ≥50% (2-3) of their forested range, but the other compromise scenarios (3b & 3c) would drive 1-3 spp (Figure 4). 
...
ggsave('output/results/fig4_scen_rangeloss_histogram_3var_shaded_countries_xytitle.png', scen_histC, width=6.85, height=4, units="in", dpi=300)
...
cowplot::save_plot("output/results/fig4_plotlabs.png", scen_histC_labs, base_width=6.85, base_height = 4, dpi=300)


##### attempt at single figure ####
str(hist_spprangeloss_df_0_highC)

hist_spprangeloss1 <- hist_spprangeloss_df_0_highC %>% mutate(scenario=recode(scenario, "1-Production" = "A     1-Production", "3a-Compromise Biodiversity" = "D     3-Compromise") )
hist_spprangeloss2 <- hist_spprangeloss_df_0_highC %>% mutate(scenario=recode(scenario, "1-Production" = "B     1-Production", "3a-Compromise Biodiversity" = "D     3-Compromise") )
...
# country scen:
# Amnirana fonensis DD amphibian (ori range 1) Guinea endemic  *scen3 also affects
# Amnirana occidentalis LC amphibian (ori range 12) Range strateches from eastern Sierra Leone to western Ghana
# Crocidura nimbae NT mammal (59) This lowland to submontane West African species has been recorded from Sierra Leone, Liberia, Guinea and Côte d'Ivoire.
# Crocidura zaphiri DD mammal (15) ethiopia kenya - Zaphir's Shrew *scen3 also affects
# Hipposideros marisae VU mammal (28)
# Rhinolophus ziama EN mammal (2) *scen3 also affects

# unrestricted scen:
# Rhinolophus ziama (ori range 2)
# Hipposideros marisae (ori range 28)
# Crocidura zaphiri  (ori range 15)- DD- ethiopia kenya - Zaphir's Shrew

# below from before 27 apr 2020 
# Phrynobatrachus annulatus (Ringed River Frog), Cote d Ivoire, Ghana, bit in Liberia/Guinea - EN -
# scen 1 affects Amnirana occidentalis (LC), scen 3 does but lesser extent, ori range (20->12). Range strateches from eastern Sierra Leone to western Ghana
# scen4 affect Dobsonia pannietensis (ori range 45, all in PNG) 

#dput(unique(spprangeloss_df_0$rangeloss.class))

checkC %>% group_by(scenario, ori_range) %>% summarize(n=n())
