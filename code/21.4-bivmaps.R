# ###########################################################################################  #
### COMPUTE AREAS OF COMPROMISE (MAKE DFS FOR BIVARIATE CHLOROPLETH MAP) --------------
...
#library(classInt) #only if using code from link above
library(dplyr)
library(sf)
library(raster)
library(ggplot2)
...
mat_vuln_list <- list.files('output/afr/biodiversity_rasters_cci/', full.names=FALSE)
...
system.time(
  for (i in 1:length(mat_vuln_list)){
    mat_vuln_rescale <- raster(paste0('output/afr/biodiversity_rasters_cci/', mat_vuln_list[i]))
...
mapply(function(dfslistname, dfslist) 
  write.csv(dfslist, file = paste0("output/dfs_for_figs/afr_cci/", dfslistname), row.names = FALSE), 
...
gc()
rm(list=setdiff(ls(), c("GISfolder", "zz", "myColors_manual", "afr_countries_wgs")))
...
res_df_list <- list.files('output/dfs_for_figs/afr_cci/', full.names=FALSE)
...
system.time(
  for (i in 1:length(res_df_list_comb)){
    res_df0 <- read.csv(paste0('output/dfs_for_figs/afr_cci/', res_df_list_comb[i]))
    ...
    ggfiglist_afr_comb[[i]] <- ggplot() +
      geom_raster(data=res_df0, aes(y=round(y, 10), x=round(x, 10), fill=bivcol)) +
...
res_df9 <- read.csv(paste0('output/dfs_for_figs/afr_cci/', res_df_list[9]))
res_df3 <- read.csv(paste0('output/dfs_for_figs/afr_cci/', res_df_list[3]))
res_df6 <- read.csv(paste0('output/dfs_for_figs/afr_cci/', res_df_list[6]))
res_df12 <- read.csv(paste0('output/dfs_for_figs/afr_cci/', res_df_list[12]))
...
res_df11 <- read.csv(paste0('output/dfs_for_figs/afr_cci/', res_df_list[11])) #data with issues
res_df2 <- read.csv(paste0('output/dfs_for_figs/afr_cci/', res_df_list[2])) #data no issues
res_df5 <- read.csv(paste0('output/dfs_for_figs/afr_cci/', res_df_list[5])) #data no issues
res_df8 <- read.csv(paste0('output/dfs_for_figs/afr_cci/', res_df_list[8])) #data no issues
all.equal(res_df5$x, res_df6$x, tolerance=0) #"Mean relative difference: 9.537867e-08"
all.equal(res_df5$y, res_df6$y, tolerance=0) #"Mean relative difference: 1.205997e-07"

#compare two problematic datasets, not the same
all.equal(res_df0$x, res_df5$x, tolerance=0) #"Mean relative difference: 8.412195e-08"
all.equal(res_df0$y, res_df5$y, tolerance=0) # "Mean relative difference: 1.264051e-07"

# datasets with no issues have the same x and y
all.equal(res_df2$x, res_df5$x, tolerance=0) #TRUE
all.equal(res_df2$x, res_df8$x, tolerance=0) #TRUE
all.equal(res_df2$y, res_df5$y, tolerance=0) #TRUE
all.equal(res_df2$y, res_df8$y, tolerance=0) #TRUE

# even for vulnA vs vulnT , have same x and y
all.equal(res_df2$x, res_df3$x, tolerance=0) #TRUE
all.equal(res_df2$y, res_df3$y, tolerance=0) #TRUE
...
res_df9$y <- res_df3$y
head(res_df9)
write.csv(res_df9, paste0('output/dfs_for_figs/afr_cci/', res_df_list[9]), row.names=FALSE)

res_df11 <- read.csv(paste0('output/dfs_for_figs/afr_cci/', res_df_list[11]))
res_df11$x <- res_df3$x
res_df11$y <- res_df3$y
write.csv(res_df11, paste0('output/dfs_for_figs/afr_cci/', res_df_list[11]), row.names=FALSE)


# Loop
system.time(
  for (i in 1:length(res_df_list)){
    res_df0 <- read.csv(paste0('output/dfs_for_figs/afr_cci/', res_df_list[i]))
    colnames(res_df0)[3] <- 'bivcol'
    res_df0$bivcol <- as.character(res_df0$bivcol)
...
# troubleshooting error in geom_raster, column infinity ###
res_df7 <- read.csv(paste0('output/dfs_for_figs/ssea/', res_df_list[7]))
res_df1 <- read.csv(paste0('output/dfs_for_figs/ssea/', res_df_list[1]))
res_df4 <- read.csv(paste0('output/dfs_for_figs/ssea/', res_df_list[4]))
res_df10 <- read.csv(paste0('output/dfs_for_figs/ssea/', res_df_list[10]))
...
all.equal(res_df7$x, res_df1$x, tolerance=0) #"Mean relative difference: 1.42598e-07"
all.equal(res_df7$y, res_df1$y, tolerance=0) #"Mean relative difference: 1.085892e-07"

# datasets with no issues have the same x and y
all.equal(res_df1$x, res_df9$x, tolerance=0)
all.equal(res_df1$y, res_df9$y, tolerance=0)
...
res_df7$y <- res_df1$y
head(res_df7)
write.csv(res_df7, paste0('output/dfs_for_figs/ssea/', res_df_list[7]), row.names=FALSE)
...
#ggfigrich_ssea[[3]] #no more issues
...
afr_pa_df <- read.csv('output/dfs_for_figs/afr_cci/pa_df_LU_cci.csv')
...
cowplot::save_plot("output/results/fig2_ssea_afr_comb_multipanel_vulnT_threat_std_pa.png", gg_topbottom2, base_height = overall_height+0.05, base_width=6.85) 
...
cowplot::save_plot("output/results/docS2_fig2_ssea_afr_comb_multipanel_vuln_all_std_pa.png", gg_topbottom2, base_height = overall_height+0.05, base_width=6.85) 
...
cowplot::save_plot("output/results/ssea_afr_comb_multipanel4_richness_std_pa.png", gg_topbottom2, base_height = overall_height+0.05, base_width=6.85)
...
gg_aoc_legend_vuln <- gg_aoc_legend_vuln + 
  #geom_rect(data=aoc.rect, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), fill=NA, color=gray(0.8), lty=1, lwd=0.7, inherit.aes = FALSE) +
  #geom_rect(data=aor.rect, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), fill=NA, color=gray(0.5), lty=1, lwd=0.7, inherit.aes = FALSE) +
...
system.time(
  for (i in 1:length(res_df_list)){
    res_df <- read.csv(paste0('output/dfs_for_figs/afr_cci/', res_df_list[i]))
    colnames(res_df)[3] <- 'bivcol'
...
system.time(cowplot::save_plot("output/results/figS1_aoc_aor_conc2.png", gg_aoc_aor_conc, base_width=6.85, base_height = 6.85*(rel_h_afr+rel_h_ssea), dpi=300)) # 19s
...
system.time(cowplot::save_plot("output/results/docS2_reference_aoc_aor_conc.png", gg_aoc_aor_conc_vulnA, base_width=6.85, base_height = 6.85*(rel_h_afr+rel_h_ssea), dpi=300)) # 19s
...
system.time(ggsave('./output/results/figS4_scen_explainer.png', plot=gg_scen_supp, height=174/3, width=174, units='mm', dpi=300)) #0.33s
#system.time(cowplot::save_plot("output/results/figs4_scen_explainer.png", gg_scen_supp, base_width=3.35, base_height=3.35/3, dpi=300)) #0.28s



### Xtra fig: AOC/AOR only for subplots ####
zz4
zz4 <- zz3
zz4$color <- ifelse(zz4$vuln==0.8, zz4$color, NA)
...
res_df_list <- list.files('output/dfs_for_figs/afr_cci/', full.names=FALSE)
res_df_list <- res_df_list[grep("biv20_df_", res_df_list)]
res_df_list <- res_df_list[grep("combspp", res_df_list, invert=TRUE)]
res_df_list <- res_df_list[grep("ext", res_df_list, invert=TRUE)]
res_df_list <- res_df_list[grep("threat", res_df_list)]
...
system.time(
  for (i in 1:length(res_df_list)){
    res_df <- read.csv(paste0('output/dfs_for_figs/afr_cci/', res_df_list[i]))
    colnames(res_df)[3] <- 'bivcol'
    res_df <- left_join(res_df, zz4, by=c('bivcol'))
    res_df$bivcol <- as.character(res_df$bivcol)
...
gg_aoc_aor_sub_afr <- plot_grid(ggfiglist_afr_aoc[[1]], ggfiglist_afr_aoc[[2]], ggfiglist_afr_aoc[[3]], ggfiglist_afr_aoc[[4]],  ncol=1,  label_size=8, align="v", axis="r")

gg_aoc_aor_sub_ssea <- plot_grid(ggfiglist_ssea_aoc[[1]], ggfiglist_ssea_aoc[[2]], ggfiglist_ssea_aoc[[3]], ggfiglist_ssea_aoc[[4]],  ncol=1,  label_size=8, align="v", axis="r")

system.time(cowplot::save_plot("output/results/xtrafig_aoc_aor_subplots_afr.png", gg_aoc_aor_sub_afr, base_width=3, base_height = 9, dpi=300)) # 19s base_width=3, base_height = 9, 

system.time(cowplot::save_plot("output/results/xtrafig_aoc_aor_subplots_ssea.png", gg_aoc_aor_sub_ssea, base_width=3, base_height = 6,  dpi=300)) # 43s
