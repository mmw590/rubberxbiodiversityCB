######## 06-scenarios_hpc_scen2_vulnA.R ############
...
# Scenario 2d: Conservation-driven, order of increasing vulnA & aboveground carbon ####
scen_in <- suit_vuln_vals3[ , .(suit, forest, avg.vulnA.carb)]
scen_in <- data.matrix(scen_in) #2.9mb as matrix

rm(list=setdiff(ls(), c("scen_results", "scen_in", "scen_spp", "spp_ranges", "res_0", "nrep", "ncores")))
gc()

#### McLapply method ####
scen_suit_rep <- mclapply(1:nrep, function(i, ...) {scen_results(vals_mod=scen_in, vals_spp=scen_spp, spp_ranges=spp_ranges)} , mc.cores=ncores, mc.set.seed=TRUE)
...
fwrite(scen_df, 'output/scen_df_vulnA_2.csv')
...
fwrite(spprangeloss_df, 'output/spprangelosstbl_vulnA_scen2.csv')
