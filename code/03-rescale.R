...
rlist <- list.files('output/afr/biodiversity_rasters/', full.names = TRUE)
...
rstacknames <- sub("output/afr/biodiversity_rasters/", "" , rlist)
...
range(values(richness_comb_rescale_ssea), na.rm=TRUE) #[1] 0.018249 1.000000
...
range(values(vuln_comb_rescale_ssea), na.rm=TRUE) #[1] 0.0400636 1.0000000
...
print(range_vulnT_rescaled)
# [1,] 0.05041408 0.5656473
# [2,] 0.00000000 0.6944444
# [3,] 0.11594203 1.0000000
# [4,] 0.00000000 1.0000000
...
range(values(vulnT_comb_rescale_afr), na.rm=TRUE) #[1] 0.01921842 1.00000000
...
range(values(vulnT_comb_rescale_ssea), na.rm=TRUE) #[1] 0.08158289 0.91536217
