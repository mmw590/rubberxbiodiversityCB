####### >>>> 21.3-numbersforcell-suit_ext.R <<<<< ##########
rm(list=ls())
...
### + Africa ####
list.files('output/afr/biodiversity_rasters_cci/')
mat_rich <- raster('output/afr/biodiversity_rasters_cci/rescaled_twice_afr_combspp_richness_std_mask.tif')
mat_vulnA <- raster('output/afr/biodiversity_rasters_cci/rescaled_twice_afr_combspp_vuln_all_std_mask.tif')
mat_vulnT <- raster('output/afr/biodiversity_rasters_cci/rescaled_twice_afr_combspp_vuln_threat_std_mask.tif')
...
mat_op <- raster('output/afr/various_rasters/op_suitability_afr.tif')
mat_acc <- raster('output/afr/various_rasters/accessibility_afr.tif')
mat_carb <- raster('output/afr/various_rasters/carbon_afr.tif')
#mat_carb <- raster('output/afr/various_rasters/carbon_afr_cropwgs.tif') #makes no diff in the sums
...
### Stack
suit_vuln_stack_afr <- stack(mat_suit, mat_pa, mat_rich, mat_vulnA, mat_vulnT, mat_forest, mat_op, mat_acc, mat_carb, mat_country)
...
suit_vuln_vals_afr <- suit_vuln_vals_afr %>% 
  rename(suit = names(suit_vuln_vals_afr)[1], 
         ...
         op = names(suit_vuln_vals_afr)[7],
         acc = names(suit_vuln_vals_afr)[8],
         carb = names(suit_vuln_vals_afr)[9],
         countryID = names(suit_vuln_vals_afr)[10]   ) %>%
...
all.equal(countryNA_corr[ , 3:11], countryNA[, 1:9]) #[1] "Component “acc”: Mean relative difference: 1.181546e-06"
...
suit_vuln_vals_afr2 %>% dplyr::filter(is.na(.$countryID)==TRUE & is.na(.$countryID.corrected)==FALSE) %>% nrow() #44, correct
...
unique(suit_vuln_vals_afr2$countryID) #58 countries
...
mat_op <- raster('output/ssea/various_rasters/op_suitability_ssea.tif')                                                    
...
### Stack raster
suit_vuln_stack_ssea <- stack(mat_suit, mat_pa, mat_rich, mat_vulnA, mat_vulnT, mat_forest, mat_op, mat_acc, mat_carb, mat_country, mat_conc )
...
suit_vuln_vals_ssea <- suit_vuln_vals_ssea %>% 
  rename(suit = names(suit_vuln_vals_ssea)[1], 
         ...
         op = names(suit_vuln_vals_ssea)[7],
         acc = names(suit_vuln_vals_ssea)[8],
         carb = names(suit_vuln_vals_ssea)[9],
         countryID = names(suit_vuln_vals_ssea)[10],
         conc = names(suit_vuln_vals_ssea)[11]) %>%
  mutate(region = 'ssea')
...
all.equal(countryNA_corr[ , 3:11], countryNA[, 1:9]) #[1] "Component “acc”: Mean relative difference: 2.469414e-08"
countryNA <- cbind(countryNA, countryID.corrected = countryNA_corr$countryID.corrected)
...
unique(suit_vuln_vals_ssea2$countryID) #38 countries

# Replace countryID with country code
countrycode <- read.csv("output/ssea/various_rasters/ssea_countryID.csv")

#countrycode <- countrycode %>% filter(NAME_0 %in% c("China", "Thailand", "Indonesia", "Malaysia", "India", "Vietnam", "Cambodia", "Myanmar", "Laos", "Sri Lanka", "Philippines") )

suit_vuln_vals_ssea2 <- left_join(suit_vuln_vals_ssea2, countrycode) %>% rename(country = NAME_0)
unique(suit_vuln_vals_ssea2$country) #38 countries
...
suit_vuln_vals2 <- suit_vuln_vals %>% 
  ...
  dplyr::filter(suit > 0) #281150 (May2020); 242117 (Feb/Apr2020)
...
suit_vuln_vals3 <- suit_vuln_vals %>% 
  ...
  dplyr::filter(suit > 0) %>% 
  dplyr::filter(pa == 0) %>%
  dplyr::filter(conc == 0) #exclude PAs and conc #236117 in May2020 / 208253 in Feb/Apr2020 (27 Apr 2020 172288)

suit_vuln_vals3 <- dplyr::select(suit_vuln_vals3, -pa, -conc)


#### Write suit_vuln_vals3.csv  ####
fwrite(suit_vuln_vals3, 'output/suit_vuln_vals3_may2020.csv')
...
write.csv(tblS1, 'output/results/TableS1_may2020.csv', row.names=FALSE)
...
dflist <- list.files("data/", "_spp_in_", full.names=TRUE)
dflist <- grep( "threat", dflist,  value=TRUE)
dflist <- grep( "threat", dflist,  value=TRUE, invert=TRUE)

spplist <- lapply(dflist[c(1:2)], read.csv)
spplist <- rbindlist(spplist)
length(unique(spplist$x)) #789 amph threat, 2534

spplist <- lapply(dflist[c(3:4)], read.csv)
spplist <- rbindlist(spplist)
length(unique(spplist$x)) #737 birds, 6088

spplist <- lapply(dflist[c(5:6)], read.csv)
spplist <- rbindlist(spplist)
length(unique(spplist$x)) #1055 mammals, 3136

spplist <- lapply(dflist[c(7:8)], read.csv)
spplist <- rbindlist(spplist)
...
gg_bin_vulnT_afr <- ggplot(data=suit_vuln_vals_gg_afr, aes(y=suit, x=vulnT)) + 
  ...
  scale_y_continuous(breaks=seq(0,1,0.2), limits=c(-0.05,1), expand=expand_scale(mult=0, add=0.05), labels=c(0,0.2,0.4,0.6,0.8,'')) + 
...
cowplot::save_plot("output/results/fig1_bin2d_0.01_reverse.png", gg_bin, base_width=6.85, base_height=6.85/2, dpi=300)
...
cowplot::save_plot("output/results/textS2_fig1_bin2d_0.01_reverse.png", gg_bin, base_width=6.85, base_height=6.85/2, dpi=300)
...
#### /OLD + Fig 1 (CB first submission) ####

suit_vuln_vals_gg <- suit_vuln_vals3 %>% dplyr::select(suit, richness, vulnA, vulnT, region) %>% filter(is.na(.$suit)==FALSE & (is.na(.$richness)==FALSE | is.na(.$vulnA)==FALSE | is.na(.$vulnT)==FALSE) )

suit_vuln_vals_gg_afr <- filter(suit_vuln_vals_gg, region=='afr')
suit_vuln_vals_gg_ssea <- filter(suit_vuln_vals_gg, region=='ssea')


gg_scatterbin_vulnT_afr <- ggplot(data=suit_vuln_vals_gg_afr) + 
  geom_point(aes(y=suit, x=vulnT),  size=1, alpha=0.5,  shape=1, colour=hue_pal()(1)) +
  geom_hline(yintercept=seq(0.2,0.8,0.2), linetype=2, alpha=0.3) +
  geom_vline(xintercept=seq(0.2,0.8,0.2), linetype=2, alpha=0.3) +
  scale_x_continuous(breaks=seq(0,1,0.2), limits=c(0,1), expand=expand_scale(mult=0, add=0.02)) + 
  scale_y_continuous(breaks=seq(0,1,0.2), limits=c(0,1), expand=expand_scale(mult=0, add=0.02), labels=c(0,0.2,0.4,0.6,0.8,'')) + 
  ylab('Rubber bioclimatic suitability') + xlab('Extinction vulnerability') +
  theme_cowplot(font_size=10) +
  theme(legend.position="none")

gg_scatterbin_vulnT_ssea <- ggplot(data=suit_vuln_vals_gg_ssea) + 
  geom_point(aes(y=suit, x=vulnT), size=1, alpha=0.5,  shape=2, colour=hue_pal()(2)[2]) +
  geom_hline(yintercept=seq(0.2,0.8,0.2), linetype=2, alpha=0.3) +
  geom_vline(xintercept=seq(0.2,0.8,0.2), linetype=2, alpha=0.3) +
  scale_x_continuous(breaks=seq(0,1,0.2), limits=c(0,1), expand=expand_scale(mult=0, add=0.02)) + 
  scale_y_continuous(breaks=seq(0,1,0.2), limits=c(0,1), expand=expand_scale(mult=0, add=0.02), labels=c(0,0.2,0.4,0.6,0.8,'')) + 
...
aoc.rect <- data.frame(xmin=0, xmax=0.39, ymin=0.41, ymax=1)
aoc.text <- data.frame(x=c(0.2, 0.2), y=c(0.7,0.5), text=c("Area of", "Compromise" ))

ww.rect <- data.frame(xmin=0.01, xmax=0.19, ymin=0.81, ymax=0.99)
ww.text <- data.frame(x=0.1, y=0.9, text="Win-\nWin")

gg_fig1a_scatter <- gg_scatterbin_vulnT_afr + 
  geom_rect(data=ww.rect, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), fill=NA, color=gray(0.5), lty=1, lwd=0.7, inherit.aes = FALSE, alpha=0.7) +
  geom_rect(data=aoc.rect, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), fill=NA, color=gray(0.1), lty=1, lwd=0.7, inherit.aes = FALSE, alpha=0.7) +
  geom_rect(data=aor.rect, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), fill=NA, color=gray(0.5), lty=1, lwd=0.7, inherit.aes = FALSE, alpha=0.7) +
  geom_text(data=ww.text,  aes(x=x, y=y,label=text), inherit.aes=FALSE, color=gray(0.5), size=4) +
  geom_text(data=aoc.text, aes(x=x, y=y,label=text), inherit.aes=FALSE, color=gray(0.1), size=4) +
  geom_text(data=aor.text, aes(x=x, y=y,label=text), inherit.aes=FALSE, color=gray(0.5), size=4) 

gg_fig1b_scatter <- gg_scatterbin_vulnT_ssea + 
  geom_rect(data=ww.rect, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), fill=NA, color=gray(0.5), lty=1, lwd=0.7, inherit.aes = FALSE, alpha=0.7) +
  geom_rect(data=aoc.rect, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), fill=NA, color=gray(0.1), lty=1, lwd=0.7, inherit.aes = FALSE, alpha=0.7) +
  geom_rect(data=aor.rect, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), fill=NA, color=gray(0.5), lty=1, lwd=0.7, inherit.aes = FALSE, alpha=0.7) +
  geom_text(data=ww.text,  aes(x=x, y=y,label=text), inherit.aes=FALSE, color=gray(0.5), size=4) +
...
#system.time(cowplot::save_plot("output/results/scatterbin_suit_vulnT_fig1c_test.png", gg_fig1c_scatter, base_width=3.35, base_height=3.35)) #200s for color; 16 for BW

gg_scatterbin <- plot_grid(gg_fig1a_scatter, gg_fig1b_scatter, labels=c('A', 'B'), label_size=10,  
                           align = 'h', nrow=1, ncol=2, rel_widths = c(1, 1)) #+ 

#system.time(ggsave('./output/results/scatterbin_suit_vulnT_afr_ssea.png', plot=gg_scatterbin, height=85, width=174, units='mm', dpi=300)) #928s=15min, BW=8s
system.time(cowplot::save_plot("output/results/fig1_scatterbin.png", gg_scatterbin, base_width=6.85, base_height=6.85/2, dpi=300)) #383s, 90s with suit_ext




#### /OLD + Text S1 Parallel Fig I for vuln A (CB first submission) ####
suit_vuln_vals_gg <- suit_vuln_vals3 %>% dplyr::select(suit, richness, vulnA, vulnT, region) %>% filter(is.na(.$suit)==FALSE & (is.na(.$richness)==FALSE | is.na(.$vulnA)==FALSE | is.na(.$vulnT)==FALSE) )
...
#vulnA
gg_scatterbin_vulnA_afr <- ggplot(data=suit_vuln_vals_gg_afr) + 
  geom_point(aes(y=suit, x=vulnA),  size=1, alpha=0.5,  shape=1, colour=hue_pal()(1)) +
  geom_hline(yintercept=seq(0.2,0.8,0.2), linetype=2, alpha=0.3) +
  geom_vline(xintercept=seq(0.2,0.8,0.2), linetype=2, alpha=0.3) +
  scale_x_continuous(breaks=seq(0,1,0.2), limits=c(0,1), expand=expand_scale(mult=0, add=0.02)) + 
  scale_y_continuous(breaks=seq(0,1,0.2), limits=c(0,1), expand=expand_scale(mult=0, add=0.02), labels=c(0,0.2,0.4,0.6,0.8,'')) + 
...
gg_scatterbin_vulnA_ssea <- ggplot(data=suit_vuln_vals_gg_ssea) + 
  geom_point(aes(y=suit, x=vulnA), size=1, alpha=0.5,  shape=2, colour=hue_pal()(2)[2]) +
  geom_hline(yintercept=seq(0.2,0.8,0.2), linetype=2, alpha=0.3) +
  geom_vline(xintercept=seq(0.2,0.8,0.2), linetype=2, alpha=0.3) +
  scale_x_continuous(breaks=seq(0,1,0.2), limits=c(0,1), expand=expand_scale(mult=0, add=0.02)) + 
  scale_y_continuous(breaks=seq(0,1,0.2), limits=c(0,1), expand=expand_scale(mult=0, add=0.02), labels=c(0,0.2,0.4,0.6,0.8,'')) + 
...
gg_figIa_vulnA <- gg_scatterbin_vulnA_afr + 
  geom_rect(data=ww.rect, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), fill=NA, color=gray(0.5), lty=1, lwd=0.7, inherit.aes = FALSE, alpha=0.7) +
  geom_rect(data=aoc.rect, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), fill=NA, color=gray(0.1), lty=1, lwd=0.7, inherit.aes = FALSE, alpha=0.7) +
  geom_rect(data=aor.rect, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), fill=NA, color=gray(0.5), lty=1, lwd=0.7, inherit.aes = FALSE, alpha=0.7) +
  geom_text(data=ww.text,  aes(x=x, y=y,label=text), inherit.aes=FALSE, color=gray(0.5), size=4) +
...
gg_figIb_vulnA <- gg_scatterbin_vulnA_ssea + 
  geom_rect(data=ww.rect, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), fill=NA, color=gray(0.5), lty=1, lwd=0.7, inherit.aes = FALSE, alpha=0.7) +
  geom_rect(data=aoc.rect, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), fill=NA, color=gray(0.1), lty=1, lwd=0.7, inherit.aes = FALSE, alpha=0.7) +
  geom_rect(data=aor.rect, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), fill=NA, color=gray(0.5), lty=1, lwd=0.7, inherit.aes = FALSE, alpha=0.7) +
  geom_text(data=ww.text,  aes(x=x, y=y,label=text), inherit.aes=FALSE, color=gray(0.5), size=4) +
...
gg_scatterbin_vulnA <- plot_grid(gg_figIa_vulnA, gg_figIb_vulnA, labels=c('A', 'B'), label_size=10,  
                                 align = 'h', nrow=1, ncol=2, rel_widths = c(1, 1)) #+ 

system.time(cowplot::save_plot("output/results/docS2_fig1_vulnA.png", gg_scatterbin_vulnA, base_width=6.85, base_height=6.85/2, dpi=300)) #383s, 229s for suit ext
